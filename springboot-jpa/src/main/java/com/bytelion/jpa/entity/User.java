 ...
 * @package com.bytelion.jpa.entity
 * @description 用户实体表
 * @date: Created in 2020/10/8 20:54
...
public class User extends AbstractAuditModel {
     ...
     * 2。@JoinTable注解的name 属性指定关联表的名字，joinColumns指定外键的名字,关联到关系维护端（User)
     * 3. inverseJoinColumns 指定外键的名字，要关联到关系被维护端（department)
    ...
    @JoinTable(name = "byte_user_dept",joinColumns = @JoinColumn(name = "user_id",referencedColumnName = "id"),inverseJoinColumns = @JoinColumn(name = "dept_id",referencedColumnName = "id"))
